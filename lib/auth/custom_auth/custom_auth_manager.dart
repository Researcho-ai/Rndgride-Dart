...
class CustomAuthManager {
  ...
  Future signOut() async {
    ...
    rndgridUserAuthUserSubject.add(
      RndgridUserAuthUser(loggedIn: false),
    );
    persistAuthData();
  }

  Future<RndgridUserAuthUser?> signIn({
    String? authenticationToken,
    String? refreshToken,
    DateTime? tokenExpiration,
    String? authUid,
  ...
  RndgridUserAuthUser? _updateCurrentUser({
    String? authenticationToken,
    String? refreshToken,
    DateTime? tokenExpiration,
    String? authUid,
    ...
    final updatedUser = RndgridUserAuthUser(
      loggedIn: true,
      uid: authUid,
      userData: userData,
    );
    rndgridUserAuthUserSubject.add(updatedUser);
    persistAuthData();
    return updatedUser;
  }
  ...
  Future initialize() async {
    ...
    try {
      ...
      userData = _prefs.getString(_kUserDataKey) != null
          ? UserProfileDataStruct.fromSerializableMap(
...
RndgridUserAuthUser? currentUser;
